...
if (isset($_GET['game_name']) && isset($_GET['gamemode']) && isset($_GET['classement_select'])) {
    ...
    switch ($classement_select) {
        case 0:
            $sql = "SELECT PRO.avatar, PRO.pseudo, CLS.score FROM game GME INNER JOIN gamemode MDE ON GME.id = MDE.id_game INNER JOIN classement CLS ON MDE.id = CLS.id_gamemode INNER JOIN profil PRO ON CLS.id_profil = PRO.id WHERE GME.nom = '$game_name' AND MDE.nom = '$gamemode_name' ORDER BY CLS.score DESC";
            break;
        case 1:
            $sql = "SELECT DISTINCT PRO.avatar, PRO.pseudo, CLS.score 
            FROM game GME 
            INNER JOIN gamemode MDE ON GME.id = MDE.id_game 
        ...
        case 2:
            ...
            INNER JOIN classement CLS ON MDE.id = CLS.id_gamemode 
            INNER JOIN profil PRO ON CLS.id_profil = PRO.id 
            INNER JOIN follow F ON CLS.id_profil = F.id_followed OR CLS.id_profil = '$id_profil' 
            WHERE GME.nom = '$game_name' AND MDE.nom = '$gamemode_name' 
                  AND (CLS.id_profil = '$id_profil' OR F.id_follower = '$id_profil')
            ORDER BY CLS.score DESC";
            break;
        case 3:
            $sql = "SELECT DISTINCT PRO.avatar, PRO.pseudo, CLS.score 
            ...
            INNER JOIN classement CLS ON MDE.id = CLS.id_gamemode 
            INNER JOIN profil PRO ON CLS.id_profil = PRO.id 
            INNER JOIN follow F ON CLS.id_profil = F.id_follower OR CLS.id_profil = '$id_profil' 
            WHERE GME.nom = '$game_name' AND MDE.nom = '$gamemode_name' 
                  AND (CLS.id_profil = '$id_profil' OR F.id_followed = '$id_profil')
            ORDER BY CLS.score DESC";
            break;
        default:
            $sql = "SELECT PRO.avatar, PRO.pseudo, CLS.score FROM game GME INNER JOIN gamemode MDE ON GME.id = MDE.id_game INNER JOIN classement CLS ON MDE.id = CLS.id_gamemode INNER JOIN profil PRO ON CLS.id_profil = PRO.id WHERE GME.nom = '$game_name' AND MDE.nom = '$gamemode_name' ORDER BY CLS.score DESC";
